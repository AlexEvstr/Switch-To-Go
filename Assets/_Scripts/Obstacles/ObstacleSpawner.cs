...
{
    [SerializeField] private GameObject _road; // Префаб участка пути
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            GameObject _platform = Instantiate(_road) as GameObject;
    ...
    void SpawnPlatform()
    {
        ...
        { // Установить префаб по оси Z
            GameObject _platform = Instantiate(_road) as GameObject;
            _platform.transform.position = lastpos + new Vector3(0f, 1f, 0f);
            lastpos = _platform.transform.position;
        }
